...
const SidebarItem = ({ item, activeTab, setActiveTab, COLORS, router }: { item: any, activeTab: string, setActiveTab: (tab: string) => void, COLORS: any, router: any }) => {
  ...
  return (
    ...
    >
      ...
      {item.badge > 0 && (
        <Badge bg="$red500" size="sm" borderRadius="$full" ml="auto" variant="solid"><BadgeText color="$white">{item.badge}</BadgeText></Badge>
      )}
    </Button>
  );
};
...
export default function AdminLayout() {
  ...
  const MENU_ITEMS = [
    ...
    { id: 'users', label: 'Manajemen User', icon: 'people', href: '/admin/users' },
    { id: 'classes', label: 'Manajemen Kelas', icon: 'calendar', href: '/admin/class-schedules' },
    { id: 'logs', label: 'Logs Aktivitas', icon: 'document-text', href: '/admin/logs' },
  ];

  if (!isReady) return null; // Prevent flickering while checking auth
            ...
            <HStack alignItems="center" mb="$4" px="$2" justifyContent="space-between">
              <HStack alignItems="center">
                <Box w={36} h={36} bg={COLORS.primary} borderRadius="$md" mr="$3" alignItems="center" justifyContent="center">
                  <Text color="white" fontWeight="bold" size="xl">A</Text>
                </Box>
                <VStack>
                  <Heading size="sm" color="$textLight900">LMS Admin</Heading>
                  <Text size="xs" color="$textLight500">Administrator</Text>
                </VStack>
              </HStack>
        ...
        <VStack flex={1} h="100%">
          ...
          <HStack bg={COLORS.primary} h={80} alignItems="center" justifyContent="space-between" px="$4" sx={{ '@md': { px: '$8' } }} shadowColor="#000" shadowOffset={{width:0, height:2}} shadowOpacity={0.1} elevation={4}>
            ...
            <HStack alignItems="center" space="md" ml="$2">
               ...
               <Avatar bg="white" size="sm"><AvatarFallbackText color={COLORS.primary}>AD</AvatarFallbackText></Avatar>
            </HStack>
          </HStack>
