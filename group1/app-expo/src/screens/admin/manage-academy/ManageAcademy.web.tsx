...
const ManageAcademyScreen = () => {
  ...
  const saveCourse = async () => {
    ...
    try {
      if (mode === 'add') {
        await axios.post(`${process.env.EXPO_PUBLIC_API_URL}/courses`, form);
      } else if (mode === 'edit' && selectedId) {
        await axios.put(`${process.env.EXPO_PUBLIC_API_URL}/courses/${selectedId}`, form);
      }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await axios.delete(`${process.env.EXPO_PUBLIC_API_URL}/courses/${id}`);
      fetchCourses();
    } catch (err: any) {
